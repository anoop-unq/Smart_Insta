...
const UserProfile = () => {
  ...
  const { posts = [], users = [], userdata, deletePost } = useContext(AppContext);
  ...
  if (!profileUser) {
    return (
      <div className="text-center py-8">
        ...
        <p>The user you're looking for doesn't exist or hasn't created any content yet.</p>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="flex items-start mb-6">
            <img 
              ...
              alt="User avatar" 
              className="w-20 h-20 rounded-full mr-4 object-cover flex-shrink-0"
            />
            <div className="min-w-0 flex-1">
              ...
              {profileUser.bio ? (
                <div className="mt-2 text-gray-700">
                  {showFullBio ? (
                    <p className="whitespace-pre-line break-words">{profileUser.bio}</p>
                  ) : (
                ...
                userdata?.user?._id === profileUser._id && (
                  <button
                    onClick={() => navigate(`/add-bio/${profileUser._id}`)}
              ...
              <div key={post._id} className="bg-white rounded-lg shadow p-4">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center min-w-0">
                    <img
                      src={post.author.avatar || assets.user_image || '/default-avatar.png'}
                      alt={post.author.name}
                      className="w-10 h-10 rounded-full mr-3 flex-shrink-0"
                    />
                    <div className="min-w-0">
                      <h3 className="font-semibold truncate">{post.author.name}</h3>
                      <p className="text-gray-500 text-sm truncate">@{post.author.username}</p>
                    </div>
                  </div>
                  {profileUser._id === post.author._id && (
      ...
      >
        <FaEdit />
                ...
                <p className="mb-4 whitespace-pre-line break-words">{post.content}</p>
                {post.image && (
                  <img
                    src={post.image}
                    alt="Post content"
